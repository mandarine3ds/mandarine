...
void RO::LoadCRO(Kernel::HLERequestContext& ctx, bool link_on_load_bug_fix) {
    ...
    if (cro_buffer_ptr & Memory::MANDARIN_PAGE_MASK) {
    ...
    if (cro_address & Memory::MANDARIN_PAGE_MASK) {
    ...
    if (cro_size & Memory::MANDARIN_PAGE_MASK) {
